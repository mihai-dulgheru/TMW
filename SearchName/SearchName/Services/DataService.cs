        ...
        private async Task<IList<Models.Result>> SearchName(string name)
        {
            using (var client = new HttpClient())
            {
                var text = await client.GetStringAsync(
                    Utilities.Constants.NameSearchEndpoint
                    + name.ToLower());
                ...
                {
                    var locationResolveEndpoint =
                        string.Format(Utilities.Constants.LocationResolveEndpoint,
                ...
                foreach (var district in countByDistrict.Keys)
                {
